...
def register_seller(
    ...
    village = db.execute("SELECT id FROM villages WHERE code = ?", (payload.village_code,)).fetchone()
    if village is None:
        raise HTTPException(status.HTTP_400_BAD_REQUEST, f"Làng nghề '{payload.village_code}' không hợp lệ.")

    existing = db.execute("SELECT id FROM seller_profiles WHERE user_id = ?", (user["id"],)).fetchone()
    if existing is not None:
        db.execute(
            "UPDATE seller_profiles SET shop_name = ?, village_id = ?, phone = ?, bio = ? WHERE user_id = ?",
    ...
    db.execute("UPDATE users SET is_seller = 1 WHERE id = ?", (user["id"],))
...
def my_products(seller: sqlite3.Row = Depends(require_seller), db: sqlite3.Connection = Depends(get_db)):
    rows = db.execute(f"{PRODUCT_SELECT} WHERE p.seller_id = ? ORDER BY p.created_at DESC", (seller["id"],)).fetchall()
    return [row_to_product(r) for r in rows]


@router.get("/stats", response_model=SellerStatsOut)
def my_stats(seller: sqlite3.Row = Depends(require_seller), db: sqlite3.Connection = Depends(get_db)):
    row = db.execute(
        """SELECT
